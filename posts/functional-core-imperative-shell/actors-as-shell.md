...
# The Design Idea
 Here's a quick reminder on the *functional core - imperative shell* architecture that I briefly introduced in the post [[a-functional-architecture]]: It requires code to live either in the shell or in the core. The shell handles side effects like IO, state or timers in an imperative way. The core contains the business logic implemented as pure functions.
