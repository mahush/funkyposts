...
*Coupling decreases*: Of course also the class hierarchy coupling goes away with FP, still functionality is build on top of each other, but when this essentially means composing pure functions all dependencies are only on function signature level which results again in dropping complexity.
