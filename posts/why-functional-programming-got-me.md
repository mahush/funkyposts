...
*Coupling decreases*: Of course also the class hierarchy coupling goes away with FP, still functionality is built on top of each other, but when this essentially means composing pure functions all dependencies are only at the function signature level which results again in reducing complexity.
